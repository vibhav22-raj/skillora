...
export default function ProfilePage() {
  ...
  const beginEdit = () => { setForm({ name: user?.name || '', email: user?.email || '', target_role: profile?.target_role || '', career_goal: profile?.career_goal || '', experience_level: profile?.experience_level || 'beginner', weekly_hours: profile?.weekly_hours || 8, learning_style: profile?.learning_style || 'mixed' }); setEditing(true); };
  ...
  return <div className="p-4 sm:p-6 max-w-6xl mx-auto space-y-6">
    ...
    <section className="rounded-2xl border border-slate-800 bg-slate-900 p-5 overflow-hidden"><div className="flex items-start justify-between gap-4 mb-5"><div><h2 className="font-semibold text-white">Learning activity</h2><p className="text-sm text-slate-400">Meaningful study activity from the last 12 weeks.</p></div><div className="text-right text-sm"><p className="text-white font-semibold">Longest: {activity?.longest_streak || 0} days</p><p className="text-slate-400">{activity?.total_active_days || 0} active days</p></div></div><div className="min-w-[600px] grid grid-flow-col grid-rows-7 gap-1 w-max">{days.map((day) => <div key={day.key} title={`${day.label}: ${day.count} activities, ${day.minutes} minutes`} aria-label={`${day.label}: ${day.count} learning activities, ${day.minutes} minutes`} className={`h-3 w-3 rounded-sm ${day.count === 0 ? 'bg-slate-800' : day.count === 1 ? 'bg-indigo-900' : day.count === 2 ? 'bg-indigo-700' : 'bg-indigo-400'}`} />)}</div></section>
    ...
    {editing && <div className="fixed inset-0 z-50 bg-black/70 p-4 flex items-center justify-center"><form onSubmit={(event) => { event.preventDefault(); save.mutate(form); }} className="w-full max-w-lg rounded-2xl border border-slate-700 bg-slate-900 p-5 space-y-4 max-h-[90vh] overflow-y-auto"><div className="flex justify-between"><h2 className="font-semibold text-white">Edit profile</h2><button type="button" onClick={() => setEditing(false)} className="text-slate-400">Cancel</button></div>{[['name','Name'],['email','Email'],['target_role','Target career'],['career_goal','Learning goal']].map(([field,label]) => <label key={field} className="block text-sm text-slate-300">{label}<input required={field === 'name' || field === 'email'} value={String(form[field] || '')} onChange={(e) => updateField(field, e.target.value)} className="mt-1 w-full rounded-xl border border-slate-700 bg-slate-800 px-3 py-2 text-white" /></label>)}<label className="block text-sm text-slate-300">Weekly learning hours<input type="number" min="1" max="80" value={Number(form.weekly_hours || 8)} onChange={(e) => updateField('weekly_hours', Number(e.target.value))} className="mt-1 w-full rounded-xl border border-slate-700 bg-slate-800 px-3 py-2 text-white" /></label><label className="block text-sm text-slate-300">Experience<select value={String(form.experience_level)} onChange={(e) => updateField('experience_level', e.target.value)} className="mt-1 w-full rounded-xl border border-slate-700 bg-slate-800 px-3 py-2 text-white"><option value="beginner">Beginner</option><option value="intermediate">Intermediate</option><option value="advanced">Advanced</option></select></label><button disabled={save.isPending} className="w-full rounded-xl bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 py-2.5 text-white font-medium">{save.isPending ? 'Saving…' : 'Save changes'}</button>{profile?.profile_image && <button type="button" onClick={() => save.mutate({ profile_image: null })} className="w-full text-sm text-rose-300 flex justify-center gap-2"><Trash2 className="h-4 w-4" /> Remove profile image</button>}</form></div>}
  </div>;
}
