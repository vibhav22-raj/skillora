  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-slate-400 mt-1">100+ free curated resources for every skill level.</p>
      </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <select value={format} onChange={(e) => { setFormat(e.target.value); setPage(1); }}
            ...
            {FORMATS.slice(1).map((f) => (
              <option key={f} value={f}>{getFormatIcon(f)} {f}</option>
            ))}
          </select>
      ...
      {/* Results count */}
      {data && (
        <p className="text-slate-400 text-sm mb-4">{data.total} resources found</p>
      )}
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {(data?.resources || []).map((resource: LearningResource, i: number) => (
              <motion.div
                ...
                className="bg-slate-900 border border-slate-800 hover:border-indigo-500/50 rounded-2xl p-5 flex flex-col transition-all">
                {/* Badges */}
                <div className="flex items-center gap-2 flex-wrap mb-3">
                  ...
                  {resource.is_free && (
                    <span className="text-xs bg-green-900/50 text-green-400 border border-green-800/50 px-2 py-0.5 rounded">Free</span>
                  )}
                  {resource.rating > 0 && (
                ...
                {/* Title */}
                <h3 className="text-white font-semibold text-sm leading-snug mb-1 flex-1 line-clamp-2">{resource.title}</h3>
                <p className="text-slate-400 text-xs mb-3">{resource.provider} · {resource.duration_hours}h</p>
                ...
                <div className="flex gap-2 mt-auto pt-2">
                  <a href={resource.url} target="_blank" rel="noopener noreferrer"
                    className="flex-1 flex items-center justify-center gap-1.5 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-medium py-2.5 rounded-xl transition-all">
                    <ExternalLink className="h-3.5 w-3.5" />
                    Open
                  </a>
                  <button onClick={() => startMutation.mutate(resource.id)}
                  ...
                  <button onClick={() => completeMutation.mutate(resource.id)}
                    className="bg-slate-800 hover:bg-green-900/50 text-slate-400 hover:text-green-400 p-2.5 rounded-xl transition-all">
